...
class InputMonitoring(events.PatternMatchingEventHandler):
    ...
    def on_created(self, event):
        ...
        if os.path.isdir(path):
            ...
            db_handler.insert_input_folder(path)

            # update our cache by ONE item!
            app_settings.input_cache.append(path)
            ...
            db_handler.insert_file(path, frame_rate, bit_depth, bit_rate)
    ...
    def on_deleted(self, event):
        ...
        if path in app_settings.input_cache:
            logger.info(f"Event: Folder {event.event_type} {path} True")
            db_handler.remove_input_folder(path)

            # update our cache by ONE item!
            app_settings.input_cache.remove(path)
            logger.info(f"Event: File removed from input cache {path}")

        else:
            try:
                db_handler.remove_file(event.src_path)
    ...
    def start_observer(self):
        ...
        observer = Observer()
        observer.schedule(InputMonitoring(), path=app_settings.input_path, recursive=True)
        observer.isDaemon()
        observer.start()
...
class OutputMonitoring(events.PatternMatchingEventHandler):
    ...
    def on_created(self, event):
        ...
        if event.is_directory is True:
            ...
            db_handler.insert_output_folder(path)

            # update our cache by ONE item!
            app_settings.output_cache.append(path)
    ...
    def on_deleted(self, event):
        ...
        if path in app_settings.output_cache:
            logger.info(f"Event: Folder {event.event_type} {path} True")
            db_handler.remove_output_folder(path)

            # update our cache by ONE item!
            app_settings.output_cache.remove(path)
    ...
    def start_observer(self):
        ...
        observer = Observer()
        observer.schedule(OutputMonitoring(), path=app_settings.output_path, recursive=True)
        observer.start()
